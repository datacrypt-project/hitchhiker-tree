...
(deftest simple-read-only-behavior
  ...
  (testing "Basic searches"
    (let [data1 (->DataNode [1 2 3 4] [1 2 3 4 5])
          data2 (->DataNode [6 7 8 9] [6 7 8 9 10])
...
(comment
  ...
  (let [v [0 5 6 1 7 11 17 12 13 18 2 3 19 20 8 9 14 10 15 4 16 18]]
                (let [sorted-set-order (into (sorted-set) v)
                      b-tree (reduce insert-key (empty-b-tree) v)
                      b-tree-order (lookup-fwd-iter b-tree Integer/MIN_VALUE)
...
(deftest insert-test
  (let [data1 (->DataNode [1 2 3] [1 2 3 4])
        root (->RootNode [] [data1])]
  ...
  (let [data1 (->DataNode [1 2 3 4] [1 2 3 4 5])
    ...
    (are [x y] (= (lookup-fwd-iter (insert-key root x) y)
         ...
         5.5 3))
  (lookup-fwd-iter
    (clojure.pprint/pprint (reduce insert-key (empty-b-tree) (range 30)))
    0
